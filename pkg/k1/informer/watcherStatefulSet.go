...
func WatchStatefulSets(conditions []v1beta1.StatefulSetCondition, matchConditions chan Condition, stopper chan struct{}, informer cache.SharedIndexInformer) {
	...
	informer.AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			mObj := obj.(*appsv1.StatefulSet)
			labels := obj.(*appsv1.StatefulSet).Labels
			CheckMatchConditionStatefulSet(mObj, labels, conditions, matchConditions)

		},
		UpdateFunc: func(old, new interface{}) {
			newObj := new.(*appsv1.StatefulSet)
			labels := new.(*appsv1.StatefulSet).Labels
			CheckMatchConditionStatefulSet(newObj, labels, conditions, matchConditions)
		},
...
func CheckMatchConditionStatefulSet(obj *appsv1.StatefulSet, labelsFound map[string]string, conditions []v1beta1.StatefulSetCondition, matchCondition chan Condition) {
	...
	for k, _ := range conditions {
		propertyExpected := ExtractStatefulSetConditionMap(&conditions[k])
		propertyFound := ExtractStatefulSetMap(obj)
